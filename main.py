...
class CafFinance(tk.Tk):
  def __init__(self):
    ...
    self.separator = ttk.Separator(self, orient="horizontal").place(relx=0., rely=0.35, relwidth=1, relheight=0.003)
...
#Register C
    ...
    #grand total------------------------------
    self.registerTotal = tk.Label(self, text="Total of all registers:", bg="#44505A", fg='white').place(relx=0, rely=0.36)
    self.grandTotal = self.totalA + self.totalB + self.totalC
    self.registerTotalLab = tk.Label(self, text = self.grandTotal)
    self.registerTotalLab.place(relx=0.15, rely=0.36)
    #float-------------------------------------
    self.floatAndNet = tk.DoubleVar()
    self.floatAndNet.trace_add("write", self.calculate)
    self.NetProfitText = tk.Label(self, text = "Net Profit (-$489 float): ", bg="#44505A", fg='white').place(relx=0, rely=0.42)
    self.NetProfitLab = tk.Label(self, text=(self.floatAndNet.get()))
    self.NetProfitLab.place(relx=0.17, rely=0.42)
    self.delete()
